...
HEADERS = {
    'Authorization': 'bearer CC7jTDTSuYOyS3W9sfVR358VmMLSVs4b',
...
def send_task_to_dream_api(style_id, prompt, target_img_path=None):
    ...
    while True:
        ...
        if state == "completed":
            ...
            with open(f"{DIR}image.jpg", "wb") as image_file:
...
def main(text):
    print(os.curdir)
    send_task_to_dream_api(style_id=3, prompt=text, target_img_path=None)
    print("DIR", DIR)
